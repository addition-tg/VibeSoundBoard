...
const SoundButton: React.FC<SoundButtonProps> = ({ sound }) => {
  ...
  const activeClasses = isPlaying 
    ? 'ring-2 ring-offset-2 ring-offset-purple-haze ring-red scale-105'
      ...
      <div className="relative z-10 flex flex-col items-center justify-end w-full h-full text-center">
        {!sound.imageUrl && (
          <div className="flex-grow flex items-center justify-center w-full">
            <div className={`w-1/2 h-1/2 max-w-8 max-h-8 transition-colors ${isPlaying ? 'text-sand-dollar' : 'text-sand-dollar/70'}`}>
              {sound.icon}
            </div>
          </div>
        )}
