...
import { GoogleGenAI as GoogleGenAIConstructor } from '@google/genai';
...
const App: React.FC = () => {
  ...
  const generateImageForSoundName = async (soundName: string, apiKeyForCall: string): Promise<string> => {
    ...
    const client = new GoogleGenAIConstructor({ apiKey: apiKeyForCall });
  ...
  const ControlButton: React.FC<{
    ...
    const variantClasses = {
      primary: 'bg-[var(--red)] hover:brightness-110 text-white',
      secondary: 'bg-[var(--purple-haze)] hover:bg-black/20 text-[var(--sand-dollar)] border border-[var(--sand-dollar)]/30',
      danger: 'bg-[var(--burgundy)] hover:brightness-110 text-white',
    };
    return (
      <button
        onClick={onClick}
        disabled={disabled}
        className={`flex items-center justify-center gap-2 px-3 py-2 text-sm font-medium rounded-lg focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-[var(--purple-haze)] focus:ring-[var(--red)] transition-all disabled:opacity-50 disabled:cursor-not-allowed ${variantClasses[variant]}`}
      >
        {children}
      </button>
    );
  ...
  const ApiErrorToast: React.FC<{ message: string; onDismiss: () => void; }> = ({ message, onDismiss }) => (
    <div className="fixed top-4 right-4 z-50 max-w-sm w-full bg-[var(--burgundy)] text-white p-4 rounded-lg shadow-2xl flex items-start gap-3 animate-fade-in-down" role="alert">
  ...
  return (
    <main className="bg-[var(--purple-haze)] min-h-screen flex flex-col items-center p-2 sm:p-4 md:p-6">
      ...
      <div className="w-full max-w-7xl mx-auto">
        <header className="text-center mb-6 sm:mb-8">
          ...
          <p className="mt-2 text-md sm:text-lg text-[var(--sand-dollar)] opacity-80">Load sounds, get AI-generated icons, and create your perfect board.</p>
        </header>
        ...
        <div className="text-center text-[var(--sand-dollar)]/70 text-xs sm:text-sm mb-6 max-w-4xl mx-auto space-y-1">
            <p>
            ...
            <p>
                <strong className="text-[var(--red)]">Pro Tip:</strong> Place a file named <code className="bg-black/40 text-white/90 px-1.5 py-0.5 rounded-md text-xs">key.txt</code> in your folder to automatically set your API key for thumbnail generation.
            </p>
        </div>
        ...
        <div className="bg-black/20 p-3 sm:p-4 rounded-2xl shadow-2xl border border-[var(--sand-dollar)]/20">
          {isLoading ? (
            <div className="flex items-center justify-center h-64">
              <p className="text-[var(--sand-dollar)] animate-pulse">{loadingMessage}</p>
            </div>
          ) : (
            <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-2 sm:gap-3">
              {sounds.length > 0 ? sounds.map((sound) => (
                ...
                <div className="col-span-full flex items-center justify-center h-64 text-center text-[var(--sand-dollar)]/60">
        ...
        <div className="mt-8 p-4 sm:p-6 bg-black/20 rounded-2xl shadow-lg border border-[var(--sand-dollar)]/20">
            <h2 className="text-xl font-bold text-white text-center mb-4">How to Get a Free API Key</h2>
            <ol className="list-decimal list-inside space-y-2 text-[var(--sand-dollar)]/90 max-w-2xl mx-auto">
                <li>
                    Go to <a href="https://aistudio.google.com/app/apikey" target="_blank" rel="noopener noreferrer" className="text-[var(--red)] font-semibold hover:underline">Google AI Studio</a> and sign in with your Google account.
                </li>
                <li>
        ...
        <footer className="mt-8 text-center text-[var(--sand-dollar)]/60 text-sm">
